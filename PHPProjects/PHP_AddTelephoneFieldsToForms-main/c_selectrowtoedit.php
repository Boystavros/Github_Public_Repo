...
if (!empty($testfordata)){
	...
	if ($webdata['sortby'] == 'telephone') {$sortby = 'telephone';}
	if ($webdata['sortby'] == 'id') {$sortby = 'id';}

$firstname= "%".$webdata['searchcolumnfirstname']."%";
$lastname= "%".$webdata['searchcolumnlastname']."%";
$email ="%".$webdata['searchcolumnemail']."%" ;
$telephone ="%".$webdata['searchcolumntelephone']."%" ;
$editid = $webdata['searchcolumnid'];
if (!empty($webdata['searchcolumnid'] )){ 
	$sql = "SELECT id, firstname, lastname, email, telephone FROM $mytable WHERE id = ? ";} 
	else {

$sql = "SELECT id, firstname, lastname, email, telephone FROM $mytable WHERE firstname LIKE ? AND lastname LIKE ? AND email LIKE ? AND telephone LIKE ? ORDER BY $sortby $sortdirection";}
if ($stmt = $dbhandle->prepare($sql)) {
if (!empty($webdata['searchcolumnid'] )){
	$stmt->bind_param("s",$editid ); }
	else{
	$stmt->bind_param("ssss",$firstname,$lastname,$email,$telephone);}
...
while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
	?>
		...
		</td><td> Name: "<?php echo htmlspecialchars($row["firstname"]). " " . htmlspecialchars($row["lastname"]); ?>" </td><td> email= "<?php echo htmlspecialchars($row["email"]); ?>"</td>
		<td> telephone= "<?php echo htmlspecialchars($row["telephone"]); ?>"</td></tr>
